...
Explores classical computer vision descriptors such as color histograms, HOG, LBP, and edge-based features.  
Establishes a baseline and highlights limitations of handcrafted approaches.

### **2. Deep Features with ResNet and ViT (02_deep_features_resnet_vit.html)**  
Extracts features using pretrained deep networks, reduces dimensionality with PCA, and trains multiple classifiers.  
The final model achieves strong performance and is suitable for real-time deployment.

### **3. Segmentation, Detection and Batch Classification (03_detection_segmentation_classif.html)**  
Foreground segmentation isolates the waste object before classification and provides batch evaluation utilities.
...
It also supports dataset expansion with hotkeys for instant ROI saving.

---

### **About the `.joblib` Model**

The final model stored as a `.joblib` file represents the core of the TrashNet.IA classification system.  
It condenses the entire training pipeline—feature extraction, dimensionality reduction, and machine-learning decisions—into a single deployable artifact.  

We created this file to ensure fast loading, reproducibility, and portability across machines without requiring heavy deep-learning frameworks at inference time.  
The model contains a PCA-compressed embedding space derived from Vision Transformer features, followed by a Histogram Gradient Boosting classifier trained on the six waste categories.  
This architecture provides near-instant predictions and efficient performance even on machines without GPU acceleration.
...
- **Accuracy:** ~0.88  
- **Macro F1-score:** ~0.83  
- High recall on common classes (paper, metal, cardboard)  
